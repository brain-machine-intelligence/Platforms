...
public class RobotAgent : Agent
{
    ...
    private const int Reward_MAX = 6;
    ...
    string writePath = "D:\\Github_\\robot_hand_data\\RobotAgentData\\";
            ...
            else
            {
                ...
                else
                {
                    ...
                    // calc reward distb;
                        ...
                        float sum_val = reward_count.Sum()+1;

                        double[] reward_distb = new double[Reward_MAX];
                        double kl_val = .0f;
                        for (int c=0; c< Reward_MAX; c++)
                        {
                            if (c == cur_rew)
                                reward_distb[c] = (reward_count[c] + 1) / (sum_val);
                            else
                                reward_distb[c] = reward_count[c] / (sum_val);
                        ...
                        if(prev_kl_val > kl_val)
                        {
                            ...
                            reward_count[cur_rew] += 1.0f;
                            collect_data = true;

                        }
                        else if (randProb < threshold[cur_rew])
                        {
                            prev_kl_val = kl_val;
                            reward_count[cur_rew] += 1.0f;
                            collect_data = true;
